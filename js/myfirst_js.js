...
let hero_level = 1,hero_life = 1000,hero_money = 20,hero_att = 10,hero_def = 10;
...
let open_exp = false,time_mark = 0;
	...
	window.onload = function(){
		...
		Context.fillText(hero_level,207,105);

		Context.fillText("HP:									 ",60,160);
		Context.fillText(hero_life.toFixed(0),170,160);

		Context.fillText("ATK:								 ",60,195);
		Context.fillText(hero_att,170,195);
		...
		Context.fillText(exp,172,300);

		Context.fillText(yellow_key,170,365);
		Context.fillText(blue_key,170,412);
		Context.fillText(red_key,170,461);
	...
	function move_solve(){
				...
				else if(mymap[now_floor][gox][goy] >= 3&&mymap[now_floor][gox][goy] <= 14)
				{
					...
					if(att_time * (monster_att[x] - hero_def) >= hero_life) 
					{
						...
						Context.drawImage(dialog,400,250);
						setTimeout("fresh_game_page()",1000)
				...
				{
					hero_att += 15;
					Context.drawImage(sword,400,250);
					setTimeout("fresh_game_page()",1000)
					mymap[now_floor][gox][goy] = 0;
					Context.drawImage(backpic[0],gox*49+305,goy*49+54);
					fresh_table();
				...
				{
					hero_def += 15;
					Context.drawImage(shield,400,250);
					setTimeout("fresh_game_page()",1000)
